...
la propia Raspberry Pi, en un solo proceso Python (`chat.py`, que incluye
`preguntas.py`/`reactor.py`/`display.py`/`voz_server.py`) más Ollama como
único servicio externo — sin depender de ninguna otra máquina.
...
El script verifica que Ollama y los 2 modelos estén listos y corre `chat.py`
en primer plano, que es donde interactúas por teclado — `preguntas.py` se
carga en memoria al arrancar `chat.py` mismo, y `voz_server.py` levanta
automáticamente una página web (ver abajo), sin nada más que levantar a mano.

No hace falta exportar `CHAT_SERVER_HOST` — apunta a `localhost` por defecto
en `llama_client.py`, que es justo lo que se necesita cuando Ollama corre en
la misma máquina.
...
Ojo: `qwen2.5:0.5b` está afinado en `llama_client.py` como clasificador de
rutas (few-shot para TRIVIA/BUSQUEDA_WEB/CHAT_LIBRE), no como modelo de
charla — úsalo como prueba de velocidad, pero esperá respuestas de menor
calidad conversacional que con `llama3.2:3b`.

## Escribirle a Ereberus desde el celular/otra PC (página web)

Además de la terminal, `chat.py` levanta automáticamente una página en
`http://<ip-de-la-pi>:8081/` con un campo de texto — lo que escribas ahí
llega al mismo lugar que si lo tipearas en la terminal (misma cola interna),
así que podés tener la terminal en la Pi y mandar mensajes desde el celular
en la misma red. `VOZ_PORT` cambia el puerto si hace falta.
...
|---|---|
| `chat.py` | Manager: enruta cada turno a Trivia/Búsqueda Web/Chat libre y lleva el loop de conversación. |
| `preguntas.py` | Búsqueda BM25 sobre `preguntas.jsonl` en memoria — sin HTTP, sin servidor aparte. |
| `workers.py` | La lógica de cada modo (Trivia, Búsqueda Web, Chat libre). |
| `llama_client.py` | Cliente HTTP hacia Ollama (`generar_respuesta()`, `enrutar()`). |
| `corrector.py` | Veredicto CORRECTO/INCORRECTO de una respuesta de Trivia (único punto que usa el LLM para eso). |
| `reactor.py` | Elige la cara de acierto/error de cada pregunta (sin LLM, de las columnas del dataset) y dispara música/desplazamiento. |
| `carrito_client.py` | Cliente HTTP hacia el carrito mecanum (ver `../carrito-mecanum-esp32/`) para los comandos de `desplazamiento`. |
| `verificador.py` | Revisa coherencia de las respuestas de Chat libre/Búsqueda Web antes de mostrarlas. |
| `cara_agente.py` | Cara de acierto/error para Chat libre/Búsqueda Web (genérica, sin fila de dataset asociada). |
...
| `voz_server.py` | Página web de texto (ver arriba) — voz todavía deshabilitada. |
...
| `start-all.sh` | Verifica Ollama+modelos y corre `chat.py`. |
